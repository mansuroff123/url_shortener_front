      ...
      <div class="max-w-6xl mx-auto">
        ...
        <header class="mb-10 text-center md:text-left flex flex-col md:flex-row justify-between items-center gap-4">
          <div>
            ...
            <p class="text-slate-500 text-sm italic">Welcome back, {username()}!</p>
          </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl shadow-slate-200/50 p-6 md:p-8 mb-8 border border-slate-100">
          <form onSubmit={handleSubmit} class="grid grid-cols-1 md:grid-cols-12 gap-5 items-end">
            <div class="md:col-span-5">
              <label class="block text-sm font-semibold mb-2 ml-1 text-slate-700">Original (URL)</label>
              <input
                type="url"
                placeholder="https://loooong-link.com/..."
                value={url()}
            ...
            <div class="md:col-span-5">
              <label class="block text-sm font-semibold mb-2 ml-1 text-slate-700">Description (optional)</label>
              <input
                type="text"
        ...
        <div class="bg-white rounded-2xl shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
            ...
            <table class="w-full text-left border-collapse">
              <thead>
                <tr class="bg-slate-50/50 border-b border-slate-100 text-slate-500 text-xs uppercase tracking-wider">
                  ...
                  <th class="px-6 py-4 font-bold">Original Link And Description</th>
                  ...
                  {(item) => (
                    <tr class="hover:bg-indigo-50/30 transition-colors group">
                      <td class="px-6 py-5 font-semibold text-indigo-600">
                        <a href={`http://localhost:5000/${item.code}`} target="_blank">
                            http://localhost:5000/{item.code}
                        </a>
                      </td>
                      <td class="px-6 py-5">
                        <div class="text-sm font-medium text-slate-900 truncate max-w-xs">{item.original_url}</div>
                        <div class="text-xs text-slate-400 mt-0.5">{item.description || "no description :)"}</div>
                      </td>
          ...
          <Show when={!data() || data().length === 0}>
            <div class="p-20 text-center">
              ...
              <p class="text-slate-400 font-medium">No links found for your account</p>
            </div>
          </Show>
        </div>
      </div>
